...
Instructions:
- Identify and describe the assets, documentation, hardware, software, access, and other resources that must be provided by the Client to enable successful completion of the project.
- Include only those items that are clearly required or implied by the information in <context>.
- Adapt the level of detail to match the context:  
  - If the context provides specific technical, logistical, or procedural requirements, include them explicitly (e.g., number of hardware sets, access timelines, escalation procedures).
  - If the context is high-level, use standard industry expectations for access, assets, and support.
- For each item, use concise, actionable language, specifying whether it is for review, development, testing, configuration, or another purpose.
- Where appropriate, note any timing, escalation, or configuration expectations (e.g., access must be provided before project start, client team responsiveness, escalation path for delays).
- Do not invent additional requirements or generic assets not supported by the context.
- Structure the section as a numbered or bulleted list for clarity.
- If warranted by the context, include notes about access, configuration, escalation, and process expectations.
...
Instructions:
- Identify and describe the governance structure and review processes that will be applied to ensure successful project completion.
- Include only governance and review elements that are:
  - Explicitly mentioned or implied in the <context>
  - Standard and necessary for the type of engagement described
  - Appropriate for the project scale, timeline, and complexity
- Common governance elements may include: status meetings (frequency), milestone reviews, deliverable reviews, escalation procedures, quality gates, compliance checks, or client sign-off processes.
- Use simple, direct language. Avoid elaborate or unnecessary governance structures unless clearly supported by the context.
- Structure as clear bullet points or short paragraphs.
- Do not assume complex review processes, audit schedules, or governance frameworks unless they are present in the context or essential for the project type.
- Focus on practical, actionable governance that ensures project success and client satisfaction.
...
Instructions:
- Identify and clearly state what work, activities, or deliverables are explicitly NOT included in this engagement.
- Include only exclusions that are:
  - Directly relevant to the type of work described in the <context>
  - Logical boundaries based on the vendor's role vs. client's responsibilities
  - Items that could reasonably be confused as included but are actually excluded
  - Activities mentioned in context but explicitly not part of the vendor's responsibilities
- Focus on specific, concrete exclusions rather than generic industry activities.
- Use clear, direct language that eliminates ambiguity about what the vendor will and will not do.
- Structure as bullet points for clarity.
- Avoid including obvious exclusions that are unrelated to the project domain (e.g., excluding "supply chain management" when the project is purely software development).
- If the context suggests the vendor is acting as an "extended team," clarify the boundaries of that relationship.
...
Instructions:
- List and describe all specific deliverables that the vendor/supplier/contractor will provide to the Client.
- For each deliverable, provide a clear, concise description, including any versioning, format, or review/approval expectations if applicable.
- **If a deliverable is a physical good (e.g., hardware, prototypes) that must be shipped, include the full ship-to address (street, city, state, country, zip) as required for logistics/tax purposes.**
- **If a deliverable is software, documentation, or any other digital/intangible item, do NOT include a ship-to address; instead, specify the expected method of delivery (e.g., electronic submission, secure file transfer, upload to client portal, etc.).**
- Only include deliverables that are required by the context; do not invent additional items.
- Structure as a numbered or bulleted list for clarity.
- If relevant, add a note that deliverables will be reviewed and verified by the client according to their quality management or review process.
...
Instructions:
- Clearly describe the project schedule, including:
   - The project start date or the process for determining it (e.g., "mutually agreed after SOW signature and PO").
   - The estimated overall project duration and/or completion date if such information is available in context.
   - If the context includes project phases or milestones (e.g., Discovery, Implementation, V&V), include a brief summary of these with estimated durations or sequence if possible.
- If specific dates are not provided, use standard best-practice language for start and end dates.
- If the context provides assumptions for start date and completion, include them; otherwise, use a range or "to be determined."
- Use clear subheadings for each part of the schedule (e.g., "Project Start Date", "Estimated Completion Date", "Phases and Milestones" if relevant).
- Omit address, resource, or deliverable details unless directly relevant to the schedule.
- Do not invent or assume project phases or dates not supported by the context.
...
Instructions:
- Describe the proposed team structure, including job titles, quantities, and locations if available.
- For each role, provide a brief description of their primary responsibilities; tailor the level of detail to what is present or implied in the context.
- Only include roles, locations, or functions that are relevant to the engagement; do not invent additional team members or roles not supported by the context.
- If the context includes or implies an extended/embedded team model, clarify the relationship and reporting structure.
- If an org chart is referenced in context, include a placeholder for it (e.g., [INSERT ORG CHART HERE]).
- Present the roles and responsibilities in a clear table or bulleted format for readability.
- Include a RASCI (Responsible, Accountable, Supporting, Consulted, Informed) matrix for key project activities if the context provides enough detail. Otherwise, include a simplified responsibility assignment or omit the RASCI.
- Only include RASCI items that are meaningful for the described project; avoid generic or unrelated activities.
- Clearly define the RASCI key.
- Adapt tone and format to match the context: use concise, actionable language and organize for client clarity.

# Data Retention
This section includes a description of how project data will be retained, managed, and used.

Instructions:
- Clearly describe how project data—including code, documentation, test results, and communications—will be retained, managed, and accessed.
- Specify the retention period based on what is provided in the context; if the period is not defined, use standard or reasonable industry language (e.g., “for the duration of the project and X months after completion”).
- State where and how data will be stored (e.g., vendor’s secure systems, client’s systems, or as mutually agreed).
- Clearly indicate who will have access to the retained data and under what circumstances.
- Mention if and when the data will be deleted or transferred to the client.
- Avoid making promises about security standards, retention duration, or data usage beyond what is supported by the context.
- Use concise and direct language, omitting unnecessary legal or compliance jargon unless required by the context.
...
Instructions:
- Identify and describe project risks that are specifically mentioned, implied, or logical based on the project context. Do not include generic risks not supported by the context.
- For each risk, clearly state:
   - The risk description (what could go wrong)
   - The estimated probability (e.g., High, Medium, Low) if known or implied
   - The mitigation strategy (how the risk will be managed or reduced)
   - The contingency plan (what will be done if the risk materializes), if information is available
- Organize the risks in a clear table or bulleted format for readability.
- Only include mitigation and contingency steps that are actionable and relevant to the described risks.
- Do not invent risks, mitigation, or contingency plans not supported or implied by the context.
...
Instructions:
- Clearly describe the process for identifying, initiating, and managing changes to the project scope, resources, timeline, or deliverables.
- Only include change management steps or requirements that are:
   - Explicitly mentioned or reasonably implied by the context
   - Standard business practice for SOWs if not otherwise specified in the context
- Specify how changes are to be documented (e.g., formal Change Order, written approval, etc.).
- If the context mentions internal or client-specific processes, reference them accordingly.
- Clarify that changes may impact schedule, cost, or resource allocation as appropriate.
- Use concise, actionable language.
- Do not include elaborate change impact analysis or approval workflows unless supported by the context.
...
Instructions:
- Clearly distinguish between “Project Assumptions” (conditions, limitations, and dependencies underlying the SOW and pricing) and “Client Responsibilities” (what the client must provide or do for successful project execution).
- For Project Assumptions:
   - List only assumptions that are stated or reasonably implied by the context (e.g., SOW coverage, pricing basis, travel cost handling, project change conditions, reporting, resource availability).
   - Include any relevant terms about cost estimates, change orders, resource availability, or limitations on scope.
   - Reference specific project names, SOW sections, or parties as appropriate.
- For Client Responsibilities:
   - List only those requirements or dependencies on the client that are in the context or directly necessary for project success.
   - Include hardware/software access, environment setup, documentation provision, responsiveness, access to key personnel, and escalation processes if present in the context.
   - Avoid generic or obvious responsibilities unless they are standard for this type of engagement.
- Use clear, actionable language and structure the section as numbered or bulleted lists.
- Do not invent assumptions, responsibilities, or legal language not present or reasonably implied in the context.
- Adapt the level of detail and tone to match the context (high-level or detailed as appropriate).
...
Instructions:
- Clearly list the specific responsibilities and resources that the Client must provide to enable successful project execution based on context and mentioned if any additional information is required.
...
Instructions:
- Provide clear contact information for both the Client and CFeynmanCompany based on the context.

# Terms and Conditions

Instructions:
- Ensure that the terms and conditions are consistent and are not truncated
